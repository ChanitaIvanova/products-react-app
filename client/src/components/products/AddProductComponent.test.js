    ...
    describe('WHEN #addProduct is invoked', () => {
        describe("WHEN the operation is successful", () => {
            it('calls addProduct and marks the add as successful', () => {
                ...
                productsService.addProduct.mockReturnValueOnce(new Promise((resolve) => {
                    resolve(true);
                }));
                const newProduct = {name: 'Product', price: 25, currency: 'BGN'};
                const wrapper = shallow(
                    <AddProduct/>,
                );
                ...
                expect(productsService.addProduct).toHaveBeenCalledWith(newProduct);
    
                return new Promise(resolve => setImmediate(resolve)).then(() => {
                    expect(wrapper.state().isLoading).toEqual(false);
                    expect(wrapper.state().product).toEqual( {name: '', price: 0, currency: 'USD'});
